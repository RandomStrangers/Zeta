...
namespace test 
{
	...
	{
		const string src = "../Zicon.ico";
		const string dst = "../../src/_CCIcon_X11.h";
		...
		public static void Main(string[] args) {
			using (StreamWriter sw = new StreamWriter(dst)) {
				...
				sw.WriteLine("");
				sw.WriteLine("static const unsigned long ZIcon_Data[] = {");
				DumpIcon(sw, 64, 64);
				DumpIcon(sw, 32, 32);
				DumpIcon(sw, 16, 16);
				sw.WriteLine("};");
				sw.WriteLine("static const int ZIcon_Size = sizeof(ZIcon_Data) / sizeof(unsigned long);");
			}
		}
	}
}
